program TS_diagram_aomori
    ...
    if(lines==1)then
        call plots(2.,13.5,13,'TS_diagram_monthly_depth_NLine.ps')
        call symbol(1.,5.5,1.,'Monthly TS Diagrams at N-Line (depth)', 0.,len('monthly ts diagrams at N line (depth)'))
    else 
        call plots(1.,13.5,13,'TS_diagram_monthly_SLine.ps')
        call symbol(1.,5.5,1.,'Monthly TS Diagrams at S-Line (depth)', 0.,len('monthly ts diagrams at S line (depth)'))
    end if
    call newpen2(3)
            ...
            do l = lines, lines
                do st = 1, stations
                    if (sal_5(y,m,l,st,depth)<30. .or. sal_5(y,m,l,st,depth)>35.) then
                        do d = 1, depth
                        sal_5(y,m,l,st,d) = 0.  !一列全部0 データが元から無い列も0だからこれに含まれる
                        end do
                    else
            ...
            do l = lines,lines
                do y = 1, years
                    do d = 1, depth
                        ...
                        if (d<=75) then; r = 0.6-0.6*real(st-1)/2.; g = 0.6-0.6*real(st-1)/2.; b = 1.-0.2*real(st)/2.
                    ...
                    else if (st>=4 .and. st<=6) then;call gmark(salco,tempco,0.05,6)
                    else if(st>=7 .and. st<=9) then;call gmark(salco,tempco,0.05,8)
                    else;end if
                    end do
                end do
            end do
        end do
        if (mod(m,4)==0) then;call plot(-3*(width+1.5),-height-1.8,-3)
