...
! USES MODULES INSTEAD OF COMMON BLOCKS
  ...
  subroutine symbol(x,y,h,isymb,ang)
   use psstat
     ...
     ! c     ica=isymb 
      ...
      10   format(f8.4," sf")
           ...
           write(ounit,'(f9.4,2x,a4)' ) ang , ' ro '  
           write(ounit,'(3a)') "(",isymb(1:len(isymb)),") sh" 
           write(ounit,'(f9.4,2x,a4)' ) -ang , ' ro ' 
           write(ounit,*) "% end symbol"
           return
  end
  
  subroutine symbolc(x,y,h,isymb,ang)
   use psstat
     ! c     character isymb*256,ica*256,ich(256)*
           character isymb*(*)
     ! c     equivalence (ica,ich)
  ...
  subroutine symbolr(x,y,h,isymb,ang)
   use psstat
     ! c     character isymb*256,ica*256,ich(256)*
        character isymb*(*)
     ! c     equivalence (ica,ich)
   ...
   10   format(f8.4," sf")
        ...
        write(ounit,'(f9.4,2x,a4)' ) ang , ' ro '  
        write(ounit,*) "(",isymb(1:len(isymb)),") stringwidth pop " 
        write(ounit,*) 'neg 0 rmoveto '
        write(ounit,'(3a)') "(",isymb(1:len(isymb)),") sh" 
        write(ounit,'(f9.4,2x,a4)' ) -ang , ' ro ' 
