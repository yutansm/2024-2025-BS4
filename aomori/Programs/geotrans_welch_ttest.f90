program geostrophic_transport_welch_t_test
    implicit none
    ...
    integer::y,m,l,n
    real::dx1,dx2,diff_av,plot_diff_av,sem,plot95,n1,n2,s1,s2,df
    real,dimension(0:30)::t95

    call plots(4.,3.,13,'/LARGE0/gr10291/nishimori2/aomori/Geostrophy/graphs/t_test/51transport_welch.ps')
    call symbolc(9.0,height1+2.,.6,'Welch t Test for Monthly Geostrophic Transport (NLine,51db)',0.,len('Welch t Test for Monthly Geostrophic Transport (NLine,51db)'))
    ...
    do n = 1,14
        ...
        if (m == 1 .or. m == 7) then
            ...
            sem = sqrt((s1**2./n1)+(s2**2./n2))  !sum of the variances of 2 distributions of means which by variance sum law is the variance of the distribution of difference of means
        ...
        print*,m,sem,df,int(df)
            plot_diff_av = diff_av*height1/2.
            plot95 = t95(int(df))*sem*height1/2.
            ...
            call number(real(n)*dx1+0.3,plot_diff_av+0.5,0.3,t95(int(df))*sem,0.,2)
            call numberc(real(n)*dx1,height1/2.+1.,0.4,df,0.,-1)
    end do
