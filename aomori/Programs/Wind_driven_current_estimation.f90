program era5
    ...
    call plots2(nnfile = 'Wind_driven_current_estimation',oopt = 'otops',mode = 'land',x = 0.,y = -4.5,h = 'ERA5 1000hPa 35 year mean')
    call plotsave('first')
    ! monthly
    do i = 1, 12
        ...
        call butler_vector(umean(ilon:iflon,ilat:iflat,i),vmean(ilon:iflon,ilat:iflat,i),width,height,arrowwidth = 0.08,scalef = .15,line_thickness = 4,g = 1.)
    ...
    call newpage('first')
        ...
        call butler_linegraph(umeanxprimelocal,width2,height2,-4.,4.,0.,.true.,tlabel = "mean u projected onto x'",maskbelow = 0.,memlabel = '[m/s]')
        call mod12_memori(13,width2,gap = 2)

        call plot(width2+2.,0.,-3)

        call butler_linegraph(vmeanyprimelocal,width2,height2,-4.,4.,0.,.true.,tlabel = "mean v projected onto y'",maskbelow = 0.) ! units are m/s
        call mod12_memori(13,width2,gap = 2)
        ...
        call butler_linegraph(winddrivenU*100.,width2,height2,-20.,20.,0.,.true.,tlabel = "wind driven d_u'",maskbelow = 0.,memlabel = '[cm/s]',memsymfreq = 10) ! units are cm/s
        ...
        call butler_linegraph(delta_eta*100.,width2,height2,-10.,10.,0.,.true.,tlabel = "d_eta (+y' direction)",maskbelow = 0.,memlabel = '[cm]',memsymfreq = 5) ! units are cm

        call plot(width2+2.,0.,-3)

        call butler_linegraph(-delta_eta*100.,width2,height2,-10.,10.,0.,.true.,tlabel = "d_eta (-y' direction)",maskbelow = 0.,memsymfreq = 5) ! units are cm
