program SSH_csv2array
    ...
    do j = 1,num_years
        ...
        filename = "../Data/SSH/data/original/" // trim(yyyy) // "tidal_utf8.csv"
        ...
        do
            ...
            do 
                ...
                if(end_pos > 4)then ! 4 digits or more eg) 1201 or 1201.0 but have to exclude condition integers like 14.0
                    read(line(start_pos:start_pos+end_pos-2),*,iostat=ios) rnum
                    if(ios/=0)then  ! if it is not a number            eg)---                 
                ...
                elseif(end_pos ==4 ) then ! 3 digits
                    read(line(start_pos:start_pos+end_pos-2),*,iostat=ios) rnum
                    if(ios/=0)then  ! if it is not a number
                        num_tokens = num_tokens + 1
                        tokens(num_tokens) = adjustl(line(start_pos:start_pos+end_pos-2))
                        start_pos = start_pos + end_pos
                    else ! is a number
                ...
                else if(end_pos == 3)then ! 2 digits eg) 14 
                        read(line(start_pos:start_pos+end_pos-2),*,iostat=ios) rnum
                        if(ios/=0)then  ! if it is not a number
                            num_tokens = num_tokens + 1
                            tokens(num_tokens) = adjustl(line(start_pos:start_pos+end_pos-2))
                ...
                else if(end_pos < 3.and.end_pos/=0)then ! 1 digits or less but not the last token eg) 5 or comma
            ...
            do i = 1, num_months
                if (i+3 <= num_tokens) then ! 3rd token is unnecessary
                    read(tokens(i+3),*, iostat=ios) rnum
                    if (ios /= 0)then 
                        SSAP%values(n,j, i) = -999.0  ! meaning, not fully a number
                        print*,'not a number ',trim(tokens(2)),i,tokens(i+3),num_tokens
                    else
                        SSAP%values(n,j, i) = rnum
                    end if
                else
                    SSAP%values(n,j, i) = -999.0  ! puts -999 on elements with non existent data
                    print*,'insufficient data ',trim(tokens(2)),i,tokens(i+3),num_tokens
                end if
            end do
            n = n + 1
        end do
