program aomori_coastal_data
    ...
    integer,parameter::month=12,seasons=4,year_start=2008,year_max=15,station_y=2,station_x=9,depth_max=300 !station_xは深層見れない時7
...
!観測data
    ...
    do line_num = 1,1
        ...
        if(m == 1) then
            ...
            call color_sal(dx,dy,sal_avarray,sal_mask,is,station_num,js,depth_max,station_x,depth_max)
...
subroutine color_sal(dx,dy,sal_avarray,sal_mask,is,station_num,js,depth_max,station_x,depth_max_1)
integer,intent(in)::is,station_num,js,depth_max,station_x,depth_max_1
real,intent(in)::dx,dy
integer,dimension(station_x,depth_max_1),intent(in)::sal_mask
real,dimension(station_x,depth_max_1),intent(in)::sal_avarray
integer::n
real::r,g,b

call pscolorK(dx,dy,sal_avarray,sal_mask,is,station_num,js,depth_max,station_x,depth_max_1,-100.,33.8,0.,0.,1.)
call pscolorK(dx,dy,sal_avarray,sal_mask,is,station_num,js,depth_max,station_x,depth_max_1,34.3,100.,1.,0.,0.)
...
call pscolorK(dx,dy,sal_avarray,sal_mask,is,station_num,js,depth_max,station_x,depth_max_1,33.8+real(n-1)/50.,33.8+real(n)/50.,r,g,blue)
end do
! print*, sal_avarray(8,200)
call newpen2(1)
call rgbk(0.,0.,0.)
call pscont3(dx,dy,sal_avarray,sal_mask,is,station_num,js,depth_max,station_x,depth_max_1,6,33.8,0.1)
