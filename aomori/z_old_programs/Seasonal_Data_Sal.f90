program aomori_data_sal
    ...
    ! character(len=*), parameter::pass_CTD = 'edited_data_51'
    character(len=*), parameter::pass_CTD = 'Yuta_edit_median_potemp'
    ...
    integer::is=1,js=1,n,q,x,memori,num,numy,numtotal,i,j,k,y,m,line_num,year,mon,stations,gain,yagain,interval=4
    real::calc_sum,first_sum,first_tri_sum,tri_sum,calc_av,first_dev,dev_sum,SD,var,g,r,b,blue,xpoints,ypoints,rosso,verde,azul,xco,yco
...
!観測data
    ...
    call plots(1.2,5.,13,'Seasonal_Data_Sal_Yuta.ps')
    call newpen2(3)
    call symbolc(13.,14.,1.,'Seasonal Data of Water Salinity',0.,len('Seasonal Data of Water Salinity'))
    call symbolc(13.,13.,.4,'using 33.8 <= sal <= 34.4 as valid data',0.,len('using 33.8 <= sal <= 34.4 as valid data'))
    ...
    do m = 1,month,3          
        do line_num = 1,2
            ...
            do x = 1, stations
                do  k = 1, depth_max
                    do y = 1, year_max
                            ! calc_sum = first_sum + sal_5(y,m,line_num,x,k)+sal_5(y,m+1,line_num,x,k)+sal_5(y,m+2,line_num,x,k) !→ 1 datam
                            ! first_sum = calc_sum   !三か月分のデータを足し合わせた。
                            ! tri_sum = sal_5(y,m,line_num,x,k)+sal_5(y,m+1,line_num,x,k)+sal_5(y,m+2,line_num,x,k)
                                do gain = 0,2
                                    if (sal_5(y,m+gain,line_num,x,k) >= 33.8 .and. sal_5(y,m+gain,line_num,x,k)<= 34.40) then
                                        calc_sum = first_sum + sal_5(y,m+gain,line_num,x,k) !→ 1 datam
                                        tri_sum = first_tri_sum + sal_5(y,m+gain,line_num,x,k)
                                        first_sum = calc_sum                                 !三か月分のデータを足し合わせた。
                                        first_tri_sum = tri_sum
                                        num = num+1                                          !ある年の三か月におけるデータ数
                                        numtotal = numtotal+1                                !15年分*3ヶ月のデータ数
                                    else 
                                    end if
                                end do
                            if (num /= 0) then
                                trimonth_av_array(y,x,k) = tri_sum/real(num)       !ある年の三か月平均 
                            else 
                                trimonth_av_array(y,x,k) = 0.
                            end if
                            num = 0                              
                            first_tri_sum =0.     !ある年の３か月分のデータの和
                    ...
                    if (real(numtotal)==0.) then
                        ...
                        g = 1.
                        b = 1.
                    else if (real(numtotal)<=30. .and. real(numtotal)/=0.) then
                        r = 1.
                        g = (real(numtotal)/30.)
                        b = 0.
                    else 
                        r = 1.-(real(numtotal)-30.)/15.
                        g = 1.
                        b = 0.
                    end if
                    xco = dx*real(x-1)
                    ...
                    !     call numberc(real(x)*dx-dx/2.,0.6,.1,real(numtotal),0.,-1)
                    ! else
                    ! end if
                ...
                call symbolc(9.*dx/2.,0.05,0.15,'Average (per mille)',0.,len('Average (per mille)'))
                call pscolorK(dx,dy,average_array,maskav,is,stations,js,depth_max,station_x,depth_max,-100.,34.0,0.,0.,1.)!使うデータは33.8から，色は34.0から
                call pscolorK(dx,dy,average_array,maskav,is,stations,js,depth_max,station_x,depth_max,34.2,100.,1.,0.,0.)!使うデータは34.4まで，色は34.2まで
                do n=3400,3420  !最初は，１月,S_line の鉛直図の作成 
                    if (n<=3410) then
                        r = (real(n)-3400.)/10.
                        g = (real(n)-3400.)/10.
                        blue = 1.
                        call pscolorK(dx,dy,average_array,maskav,is,stations,js,depth_max,station_x,depth_max,real(n)/100.,real(n+1)/100.,r,g,blue)
                    else
                        r = 1.
                        g = 1.-(real(n)-3410.)/10.
                        blue = 1.-(real(n)-3410.)/10.
                    call pscolorK(dx,dy,average_array,maskav,is,stations,js,depth_max,station_x,depth_max,real(n)/100.,real(n+1)/100.,r,g,blue)
                    end if
                end do
                call newpen2(3)
                ...
                call symbolc(9.*dx/2.,0.05,0.15,'SD (per mille)',0.,len('SD (per mille)'))    
                call pscolork(dx,dy,sd_array,masksd,is,stations,js,depth_max,station_x,depth_max,0.2,100.,1.,0.,0.)
                do n=0,20,1  !最初は，１月,S_line の鉛直図の作成 
                    if (n<=10) then
                        r = (real(n))/10.
                        g = (real(n))/10.
                        blue = 1.
                        call pscolorK(dx,dy,sd_array,masksd,is,stations,js,depth_max,station_x,depth_max,real(n)/100.,real(n+1)/100.,r,g,blue)
                    else 
                        r = 1.
                        g = 1.-(real(n)-10.)/10.
                        blue = 1.-(real(n)-10.)/10.
    ...
    call symbol(0.,0.4,0.3,'Salinity (per mille)',0.,len('Salinity (per mille)'))
    do n=0,20
        if (n<=10) then
            rosso = (real(n))/10.
            verde = (real(n))/10.
            ...
            rosso = 1.
            verde = 1.-(real(n)-10.)/10.
            azul = 1.-(real(n)-10.)/10.
        end if
            call betsqk(0.,0.,9./21.,0.3,rosso,verde,azul)
            call rgbk(0.,0.,0.)
            call plot(0.,0.,3)
            call plot(9./21.,0.,2)
            call plot(0.,0.,3)
            call plot(0.,-0.05,2)
            call plot(9./21.,0.,3)
            call plot(9./21.,-0.05,2)
                if(mod(n,1)==0) then
                call numberc(0.,-0.15,0.1,34.0+real(n)/100.,0.,2)
                else 
                end if 
    ...
    call symbol(0.,0.4,0.3,'Standard Deviation (per mille)',0.,len('Standard Deviation (per mille)'))
    do n=0,20
        if (n<=10) then
            rosso = (real(n))/10.
            verde = (real(n))/10.
            ...
            call plot(9./21.,-0.05,2)
                if(mod(n,1)==0) then
                call numberc(0.,-0.15,0.1,real(n)/100.,0.,2)
                else 
                end if 
        call plot(9./21.,0.,-3)
    end do
