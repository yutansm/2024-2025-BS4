program aomori_data
    ...
    character(len=*), parameter::pass_CTD = 'Yuta_unedited'
    ! character(len=*), parameter::pass_CTD = 'Yuta_edit_median_potemp'
    integer::i,j,k,y,m,line_num,a,b,c,year,mon,stations
...
!観測data
    ...
    do y=1,year_max
        ...
        filename=trim(pass_CTD)//'/'//trim(line_name)//'/'//trim(moon)//'/01tem'//trim(aa)//'.csv'  !01でなく51ではないかkaiketsu
        open(31,file=filename,status='old',action='read')
        do k=1,depth_max
            read(31,102) (potemp_5(y,m,line_num,i,k),i=1,station_x)!??
        end do
        close(31)
        filename=trim(pass_CTD)//'/'//trim(line_name)//'/'//trim(moon)//'/01sal'//trim(aa)//'.csv'
        open(31,file=filename,status='old',action='read')
        do k=1,depth_max
    ...
    call plots(2.,5.,13,'MonthlyAverage_Temp_Yuta.ps')
    call newpen2(3)
    call symbolc(10.5,13.,1.,'Monthly Averages of Water Temperature',0.,37)
    ...
    do m = 1,month
        do line_num = 1,2
            ...
            do x = 1, stations
                do  k = 1, depth_max
                    do y = 1, year_max
                        if (potemp_5(y,m,line_num,x,k) /= 0.0000) then
                ...
                call pscolork(dx,dy,useful_array,mask,is,stations,js,depth_max,station_x,depth_max,-100.,0.,0.,0.,1.)
                ...
                do n=1,26,1  !最初は，１月,S_line の鉛直図の作成 
                    if (n<=11) then
                        ...
                        g = (real(n)-1)/10.
                        blue = 1.
                        call pscolorK(dx,dy,useful_array,mask,is,stations,js,depth_max,station_x,depth_max,real(n-1),real(n),r,g,blue)
                    else 
                        r = 1.
                        g = 1.-(real(n)-11.)/15.
                        blue = 1.-(real(n)-11.)/15.
    ...
    call newpen2(2)
    call symbolc(2.5,0.5,0.3,'Temperature(celsius)',0.,20)
    ...
    call numberc(0.,-0.15,0.1,26.,0.,-1) !温度バー最後の目盛
    ...
    do line_num = 1,2    !参照ファイルの決定
        ...
        filename ='edited_data'//'/'//'S-line'//'/lon.csv'
        open(31,file=filename,status='old',action='read')
