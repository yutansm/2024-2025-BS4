program aomori_data_sd_temp
    ...
    real::calc_sum,first_sum,calc_av,dev_sum,first_dev,var,SD,r,blue,xpoints,ypoints,rosso,verde,azul
...
call plots(2.,5.,13,'Monthly_sd_Sal.ps')
    call newpen2(3)
    call symbolc(10.5,13.,1.,'Monthly Standard Deviation of Water Salinity',0.,len('Monthly Standard Deviation of Water Salinity'))
    ...
    do m = 1,month
        do line_num = 1,2
            ...
            do x = 1, stations
                do  k = 1, depth_max
                    do y = 1, year_max
                        if (sal_5(y,m,line_num,x,k) > 0.) then
                            calc_sum = first_sum + sal_5(y,m,line_num,x,k) !→ 1 datam
                            first_sum = calc_sum
                            num = num+1
                        else 
                            first_sum = first_sum
                        ...
                        ! calc_av = calc_sum/real(num)
                            ...
                            if (num /= 0) then
                                ...
                                do yagain = 1,year_max
                                    if(sal_5(yagain,m,line_num,x,k) >0.) then
                                        dev_sum = first_dev + (sal_5(yagain,m,line_num,x,k)-calc_av)**2
                ...
                call pscolork(dx,dy,useful_array,mask,is,stations,js,depth_max,station_x,depth_max,0.4,100.,1.,0.,0.)
                ! call pscolork(dx,dy,useful_array,mask,is,stations,js,depth_max,station_x,depth_max,26.,100.,1.,0.,0.)
                do n=0,40,1  !最初は，１月,S_line の鉛直図の作成 
                    if (n<=20) then
                        r = (real(n))/20.
                        g = (real(n))/20.
                        blue = 1.
                        call pscolorK(dx,dy,useful_array,mask,is,stations,js,depth_max,station_x,depth_max,real(n)/100.,real(n+1)/100.,r,g,blue)
                    else 
                        r = 1.
                        g = 1.-(real(n)-20.)/20.
                        blue = 1.-(real(n)-20.)/20.
    ...
    call newpen2(2)
    call symbolc(2.5,0.5,0.3,'Standard Deviation(per mille)',0.,len('Standard Deviation(per mille)'))
    ...
    ! call numberc(0.,-0.15,0.1,0.44,0.,2) !温度バー最後の目盛
