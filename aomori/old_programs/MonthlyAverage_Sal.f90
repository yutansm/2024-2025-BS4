program aomori_data
    ...
    integer::is=1,js=1,n,q,x,memori,num = 0,stations
...
!観測data
    ...
    call plots(2.,5.,13,'MonthlyAverage_Sal.ps')
    call newpen2(3)
    call symbolc(10.,13.,1.,'Monthly Averages of Water Salinity',0.,34)
    ...
    do m = 1,month
        do line_num = 1,2
            ...
            do x = 1, stations
                ...
                call pscolorK(dx,dy,useful_array,mask,is,stations,js,depth_max,station_x,depth_max,-100.,33.8,0.,0.,1.)
                call pscolorK(dx,dy,useful_array,mask,is,stations,js,depth_max,station_x,depth_max,34.2,100.,1.,0.,0.)
                do n=3380,3420,interval  !最初は，１月,S_line の鉛直図の作成 
                    if (n<=3400) then
                        r = (real(n)-3380.)/20.
                        g = (real(n)-3380.)/20.
                        blue = 1.
                        call pscolorK(dx,dy,useful_array,mask,is,stations,js,depth_max,station_x,depth_max,real(n)/100.,real(n+interval)/100.,r,g,blue)
                    else
                        r = 1.
                        g = 1.-(real(n)-3400.)/20.
                        blue = 1.-(real(n)-3400.)/20.
                    call pscolorK(dx,dy,useful_array,mask,is,stations,js,depth_max,station_x,depth_max,real(n)/100.,real(n+interval)/100.,r,g,blue)
                    end if
                end do
                call newpen2(3)
    ...
    call newpen2(2)
    call symbolc(4.,0.5,0.3,'Salinity(per mille)',0.,20)
    ...
    do n=0,40
        if (n<=20) then
            ...
            call plot(0.,-0.05,2)
                if(mod(n,2)==0) then
                call numberc(0.,-0.15,0.1,33.8+real(n)/100.,0.,2)
                else 
                end if
        else
            rosso = 1.
    ...
    ! call numberc(0.,-0.15,0.1,34.5,0.,1) !塩分バー最後の目盛

    call plot(1.5,2.,-3) ! 原点を塩分バーの隣に置く
    call plot(0.,0.,3)  !凡例枠
    call plot(0.,0.-ylength,2)
    call plot(dx*9.,0.-ylength,2)
    call plot(dx*9.,0.,2)
