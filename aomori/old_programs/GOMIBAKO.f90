program aomori_coastal_data
    ...
    real,parameter:: axis_sal_min = 33.80, axis_sal_max = 34.30, axis_sal_range = axis_sal_max - axis_sal_min  !ふさわしいデータとして扱う塩分範囲を指定できる
    real,parameter:: valid_sal_min = 30.00, valid_sal_max = 34.65

    integer,parameter::axis_temp_range = axis_temp_max-axis_temp_min,temp_range = axis_temp_max-axis_temp_min, sal_iterations = 40 

    real,parameter::xlength=5., ylength=2.5
    real,parameter::dx = xlength/real(month), temp_dy = ylength/real(axis_temp_range), sal_dy = ylength/real(sal_iterations)
...
!観測data
    ...
    call plots(2.,13.8,13,'Monthly_av_and_sd/Monthly_Data_station'//trim(station)//'_wo2017.ps')
    call newpen2(3)
    call symbol(0.,5.,1.,'Monthly Data of station'//trim(station)//'_errorbars',0.,len('Monthly Data of station x_errorbars'))
    ...
    call symbol(12.,3.5,0.3,'data of 2017 excluded',0.,len('data of 2017 excluded'))
    ...
    do line_num = 1,2
        do k = 50,200,50
            do m = 1,month
                ...
                do ypoints = 0,sal_iterations  !目盛
                    if (mod(ypoints,4) == 0 ) then
                    call plot(0.,sal_dy*(real(ypoints)),3)
                    call plot(-0.05,sal_dy*(real(ypoints)),2)
                    call numberc(-0.3,sal_dy*(real(ypoints)),0.15,axis_sal_min+(real(axis_sal_range)/real(sal_iterations))*(real(ypoints)),0.,2)
                    else 
                    end if
                end do                                                                                  !salに関する枠作り完成
                ...
                do magain = 1,month                                                                     !tempとsalに関する平均値と標準偏差をプロット
                    ...
                    sal_av_yplot = ylength*((sal_avarray(magain,station_num,k)-axis_sal_min)/real(axis_sal_range))
                    ...
                    plot_sal_SD = ylength*((sal_SD_array(magain,station_num,k))/real(axis_sal_range))    !←salは最小値が0でないので相対的に考えている。34.20-33.80==0.4
